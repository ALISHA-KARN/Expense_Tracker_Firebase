...
import {
  ...
  setMoneySpent,
} from "../Redux/Slices/AddExpenseSlices";
...
export default function AddExpense() {
  ...
  const expenseDescription = useSelector(
    (state) => state.AddExpenseSlices.expenseDescription
  );
  ...
  const TotalMoneyExpense = useSelector(
    (state) => state.AddExpenseSlices.TotalMoneyExpense
  );
      ...
      <div className={`${Theme}`}>
        {TotalMoneyExpense > 1000 ? (
          <button
            ...
            style={{ marginLeft: "70rem", marginTop: "30px" }}
          >
            Toggle Theme
          </button>
        ) : null}
        ...
        <div className={`form-container ${Theme}`}>
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              <label htmlFor="money-spent" style={{ color: "black" }}>
                Money Spent
              </label>
              <input
                type="text"
            ...
            <div className="form-group">
              <label htmlFor="expense-description" style={{ color: "black" }}>
                Expense Description
              </label>
              <input
                type="text"
        ...
        {Loading ? (
          <p style={{ display: "auto" }}>`Loading</p>
        ) : userData.length > 0 ? (
          <DisplayExpense />
        ) : (
          <h2>No Expense please add your Expense</h2>
        )}
      </div>
    </>
  );
